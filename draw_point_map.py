...
def draw_china_weighted_point_map(spark):
    ...
    # res1 = spark.sql("select ST_Point(longitude, latitude) as point from COVID_china").show(400, False)
    ...
    save_png(res_png1, './weighted_point_map1.png')
    ...
    save_png(res_png2, './weighted_point_map2.png')
    ...
    save_png(res_png3, './weighted_point_map3.png')
    spark.catalog.dropGlobalTempView("COVID_china")
...
def draw_world_weighted_point_map(spark):
    ...
    res1 = spark.sql("select ST_Point(longitude, latitude) as point from COVID_country "
                     "where ST_Within(ST_Point(longitude, latitude), "
                     "'POLYGON ((-29.367 -59.37503, 331.95591 -59.37503, 331.95591 83.986788,"
                     " -29.367 83.986788, -29.367 -59.37503))')")
    res1.createOrReplaceTempView("res1")
    res1 = spark.sql("select * from res1 where point != 'POINT (nan nan)' ")
    res1.show(20, False)
    vega1 = vega_weighted_pointmap(3000, 2000, [-29.367, -59.37503, 331.95591, 83.986788],
                                   "#EEEEEE", [2, 60], [6], 1.0, "EPSG:4326")
    res_png1 = weighted_pointmap(res1, vega1)
    save_png(res_png1, './country_weighted_point_map1.png')

    # 2
    res2 = spark.sql("select ST_Point(longitude, latitude) as point, currentConfirmedCount as c from COVID_country "
                     "where ST_Within(ST_Point(longitude, latitude), "
                     "'POLYGON ((-29.367 -59.37503, 331.95591 -59.37503, 331.95591 83.986788,"
                     " -29.367 83.986788, -29.367 -59.37503))')")
    res2.createOrReplaceTempView("res2")
    ...
    vega2 = vega_weighted_pointmap(3000, 2000, [-29.367, -59.37503, 331.95591, 83.986788],
                                   "blue_to_red", [2, 1000], [6], 1.0, "EPSG:4326")
    ...
    # 3
    res3 = spark.sql("select ST_Point(longitude, latitude) as point, currentConfirmedCount as c, "
                     "currentConfirmedCount as s from COVID_country "
                     "where ST_Within(ST_Point(longitude, latitude), "
                     "'POLYGON ((-29.367 -59.37503, 331.95591 -59.37503, 331.95591 83.986788,"
                     " -29.367 83.986788, -29.367 -59.37503))')")
    res3.createOrReplaceTempView("res3")
    ...
    vega3 = vega_weighted_pointmap(3000, 2000, [-29.367, -59.37503, 331.95591, 83.986788],
                                   "blue_to_red", [2, 1000], [5, 10], 1.0, "EPSG:4326")

    res_png3 = weighted_pointmap(res3, vega3)
    save_png(res_png3, './country_weighted_point_map3.png')
